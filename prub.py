...
dash_app.layout = html.Div([
    ...
    html.P(
        "Upload one or multiple CSV/Excel files. Then select one file to plot, choose chart type, X/Y columns, and export to PNG.",
    ...
    # Almacenamos los DataFrames en memoria (JSON).
    # Guardaremos un diccionario {filename: df_json}
    dcc.Store(id="stored-data"),

    # Galería de archivos subidos (fila de "cards")
    ...
    # Dropdowns para seleccionar columnas X e Y
    html.Div([
...
###############################################################################
# CALLBACK 1: PROCESAR LOS ARCHIVOS Y GUARDARLOS
###############################################################################
@dash_app.callback(
...
###############################################################################
# CALLBACK 2: ACTUALIZAR EL DROPDOWN DE ARCHIVOS
###############################################################################
@dash_app.callback(
...
###############################################################################
# CALLBACK 3: AL ELEGIR UN ARCHIVO, ACTUALIZAR DROPDOWNS DE COLUMNAS
###############################################################################
@dash_app.callback(
...
###############################################################################
# CALLBACK 4: GENERAR LA GRÁFICA SEGÚN EL TIPO SELECCIONADO
###############################################################################
@dash_app.callback(
    Output("graph", "figure"),
    Input("dropdown-file", "value"),
...
def update_graph(selected_file, chart_type, x_col, y_cols, store_dict):
    if not selected_file or not chart_type or not x_col or not y_cols:
        return {}
    if isinstance(store_dict, str):
        store_dict = eval(store_dict)
    df_json = store_dict.
